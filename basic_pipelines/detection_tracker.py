...
class user_app_callback_class(app_callback_class):
    def __init__(self):
        ...
        self.class_names = []  # Will be populated later
...
def app_callback(pad, info, user_data):
    ...
    for detection in detections:
        ...
        class_id.append(0)  # We're not using class IDs, so we set all to 0

    if len(xyxy) > 0:  # Only create sv_detections if there are detections
        ...
        sv_detections = user_data.tracker.update_with_detections(sv_detections)

        print(sv_detections)
        if user_data.use_frame and frame is not None:
            # Draw bounding boxes and IDs using OpenCV
            xy_counter = 0

            for xyxy, confidence, tracker_id in zip(
                sv_detections.xyxy, sv_detections.confidence, sv_detections.tracker_id
            ):
                # Convert xyxy to integer coordinates
                # bbox = [int(coord) for coord in xyxy]
                bbox_float = [float(coord) for coord in xyxy]
                bbox = [
                    int(bbox_float[0] * width),
                    int(bbox_float[1] * height),
                    int(bbox_float[2] * width),
                    int(bbox_float[3] * height),
                ]
                ...
                # label = f"{labels[xy_counter]} #{tracker_id} {confidence:.2f}"
                label = f"{labels[xy_counter]} #{tracker_id}"
        ...
        print(f"Tracked objects: {len(sv_detections)}")
    else:
        print(f"Frame count: {user_data.get_count()}")
